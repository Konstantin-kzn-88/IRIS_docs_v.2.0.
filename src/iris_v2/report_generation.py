...
def _build_replacements(
    ...
    values: dict[str, Any] = {
        ...
        "EXECUTOR_SHORT_NAME": executor.get("name"),
        "EXECUTOR_FULL_NAME": executor.get("full_name", executor.get("name")),
        ...
        "EXECUTOR_OGRNIP": executor.get("ogrnip"),
        "EXECUTOR_TEL": executor.get("tel"),
        "EXECUTOR_FAX": executor.get("fax"),
    ...
    def _generate_single(
        ...
        *,
        ...
        if "OV_AMOUNT_SECTION" in marker_names:
            try:
                rows = load_ov_amount_rows(project_root)
        ...
        if "CASUALTIES_SECTION" in marker_names:
            try:
                ...
                raise ReportGenerationError(str(exc)) from exc
        if "DAMAGE_SECTION" in marker_names:
            try:
                rows = load_damage_rows(project_root)
                if render_damage_section(document, rows):
                    filled_sections.append("DAMAGE_SECTION")
            except ReportDamageError as exc:
                raise ReportGenerationError(str(exc)) from exc
        if "FATAL_ACCIDENT_FREQUENCY" in marker_names:
            try:
                value = load_fatal_accident_frequency(project_root)
        ...
        if "COLLECTIVE_RISK_SECTION" in marker_names:
            try:
                ...
                raise ReportGenerationError(str(exc)) from exc
        if "INDIVIDUAL_RISK_SECTION" in marker_names:
            try:
                section_data = load_individual_risk_section(project_root)
        ...
        if "SUBSTANCES_BY_COMPONENT_TABLE" in marker_names:
            try:
                ...
                raise ReportGenerationError(str(exc)) from exc
        if "TOP_SCENARIOS_DESC_BY_COMPONENT" in marker_names:
            try:
        ...
        if "TOP_SCENARIOS_PF_BY_COMPONENT" in marker_names:
            try:
                ...
                raise ReportGenerationError(str(exc)) from exc
        if "TOP_SCENARIOS_FATALITIES_INJURED" in marker_names:
            try:
                rows = load_key_scenario_people_rows(project_root)
                if render_key_scenario_people(document, rows):
                    filled_sections.append("TOP_SCENARIOS_FATALITIES_INJURED")
            except ReportKeyScenariosError as exc:
                raise ReportGenerationError(str(exc)) from exc
        if "TOP_SCENARIOS_DAMAGE" in marker_names:
            try:
