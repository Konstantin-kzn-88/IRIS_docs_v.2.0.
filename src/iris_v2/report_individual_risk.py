...
class ReportIndividualRiskError(Exception):
    pass


@dataclass(frozen=True)
class IndividualRiskSection:
...
def _load_object(path: Path, missing_message: str) -> dict[str, Any]:
    try:
        ...
        raise ReportIndividualRiskError(missing_message) from exc
    except (OSError, json.JSONDecodeError) as exc:
        raise ReportIndividualRiskError(f"Не удалось прочитать {path.name}") from exc
    if not isinstance(raw, dict):
        raise ReportIndividualRiskError(
...
def _number(value: Any, label: str) -> float:
    if (
        ...
        raise ReportIndividualRiskError(f"{label} должно быть числом не меньше нуля")
    return float(value)
...
def load_individual_risk_section(
    ...
    for index, item in enumerate(results, start=1):
        ...
        codes.add(code)
        current_people = _count(
            item.get("risk_people_count"), f"Сценарий {code}: risk_people_count"
        )
        current_no_people = item.get("individual_risk_status") == "no_people"
        expected_status = "no_people" if current_people == 0 else "calculated"
        if item.get("individual_risk_status") != expected_status:
            raise ReportIndividualRiskError(
                f"Сценарий {code}: неверный individual_risk_status"
            )
        if people_count is None:
            people_count = current_people
            no_people = current_no_people
        elif people_count != current_people or no_people != current_no_people:
            raise ReportIndividualRiskError(
                f"Файл {RISK_FILE_NAME} содержит разные данные о численности людей"
            )

        values = grouped.setdefault(component, [0.0, 0.0, 0.0])
        values[0] += 1
        fatalities = item.get("individual_risk_fatalities")
        injured = item.get("individual_risk_injured")
        if current_no_people:
            if fatalities is not None or injured is not None:
                raise ReportIndividualRiskError(
                    f"Сценарий {code}: индивидуальный риск должен отсутствовать"
                )
        else:
            values[1] += _number(
                fatalities, f"Сценарий {code}: individual_risk_fatalities"
            )
            values[2] += _number(
                injured, f"Сценарий {code}: individual_risk_injured"
            )
    ...
    for index, ((name, expected), stored) in enumerate(
        ...
        stored_fatalities = stored.get("individual_risk_fatalities")
        stored_injured = stored.get("individual_risk_injured")
        if no_people:
            if stored_fatalities is not None or stored_injured is not None:
                raise ReportIndividualRiskError(
                    f"Файл {SUMMARY_FILE_NAME} устарел: индивидуальный риск "
                    f"по составляющей «{name}» должен отсутствовать"
                )
            continue
        fatalities = _number(
            stored_fatalities,
        ...
        injured = _number(
            stored_injured,
            f"Составляющая «{name}»: individual_risk_injured",
        )
    ...
    total_fatalities = summary.get("total_individual_risk_fatalities")
    total_injured = summary.get("total_individual_risk_injured")
    if no_people:
        if total_fatalities is not None or total_injured is not None:
            raise ReportIndividualRiskError(
                f"Файл {SUMMARY_FILE_NAME} устарел: итоговый индивидуальный риск "
                "должен отсутствовать"
            )
        return IndividualRiskSection(rows=(), no_people=True)

    checked_total_fatalities = _number(
        total_fatalities, "total_individual_risk_fatalities"
    )
    checked_total_injured = _number(total_injured, "total_individual_risk_injured")
    expected_total_fatalities = sum(values[1] for values in grouped.values())
    expected_total_injured = sum(values[2] for values in grouped.values())
    if not _same(checked_total_fatalities, expected_total_fatalities) or not _same(
        checked_total_injured, expected_total_injured
    ):
        raise ReportIndividualRiskError(
            f"Файл {SUMMARY_FILE_NAME} устарел: итоговый индивидуальный риск "
            f"не совпадает с {RISK_FILE_NAME}"
    ...
    rows.append(
        {
            ...
            "fatalities": f"{checked_total_fatalities:.3E}",
            "injured": f"{checked_total_injured:.3E}",
        }
    )
    return IndividualRiskSection(rows=tuple(rows), no_people=False)
...
def render_individual_risk_section(
